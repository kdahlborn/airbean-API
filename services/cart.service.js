...
export const addToCart = async (product) => {
    try {
        const existingCart = await Cart.findOne({ cartId: product.cartId });
            ...
            if (cartItem) {
                ...
                if (cartItem.qty < 1) {
                    ...
                    if (existingCart.items.length === 0) {
                        await Cart.findOneAndDelete({ cartId: cart.cartId });
                    }
                }
            } else {
                // If item does NOT exist in cart
                // if (product.qty < 1)
                //     throw new Error('qty need to be 1 or higher');
                const newCartItem = { qty, ...item }; // Remove qty property from item

                existingCart.items.push({
                    ...newCartItem,
                    qty: product.qty, // Replace qty with qty from req.body
                });
        ...
        return {
            success: true,
            cart: existingCart ? updatedCart : newCart,
        };
    } catch (error) {}
};
